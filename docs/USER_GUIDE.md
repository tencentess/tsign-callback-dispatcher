...
当回调消息未匹配到任何已启用的下游目标时，系统会记录该消息的未分发详情。点击对应记录的 **「详情」** 按钮可查看未分发原因：

<img src="./fail-dispatcher.png" alt="未分发详情" width="600" />

未分发详情弹窗展示以下信息：

- **消息基本信息** — 消息类型、消息 ID、接收时间等
- **未分发原因** — 说明消息未匹配到任何下游目标的具体原因（如事件类型不匹配、标签筛选条件不满足等）
- **消息内容** — 完整的回调消息内容，便于排查配置问题

> 💡 **提示**：如果看到大量未分发的消息，建议检查回调配置中的事件类型筛选和标签筛选条件是否过于严格，或确认是否有对应的下游目标处于启用状态。
